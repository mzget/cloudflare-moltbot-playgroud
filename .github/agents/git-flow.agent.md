---
name: git-flow
description: "Git Flow helper agent for merge operations. Use the `/toprd` command to merge the current working branch into `develop`, then merge `develop` into `main` with safety checks."
trigger: "/toprd"
...
- When the user issues `/toprd`, the agent should:
  ...
  6. Request user confirmation before pushing branches.
