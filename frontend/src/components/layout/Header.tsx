...
import { Box, Typography, Stack, Sheet, IconButton, Tooltip, Button, Dropdown, Menu as JoyMenu, MenuButton, MenuItem, Divider, Input, Chip } from '@mui/joy';
import { 
  Bell, Settings, User, Menu, Moon, Sun, LogOut, Check,
  LayoutDashboard, BarChart3, TrendingUp, Terminal, Bot, Database, Info 
} from 'lucide-react';
...
export default function Header({ activeTab, setActiveTab, reportsCount, onOpenSidebar }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const scheduleNext = () => {
      let delay = 30_000;
      if (showNotificationsRef.current) delay = 5 * 60 * 1000;
      else if (unreadCountRef.current === 0) delay = 2 * 60 * 1000;
  ...
  const menuItems = [
    { id: 'dashboard', label: t('sidebar.dashboard'), icon: <LayoutDashboard size={18} /> },
    { id: 'market', label: t('sidebar.market_intelligence'), icon: <BarChart3 size={18} /> },
    { id: 'watchlist', label: t('sidebar.watchlist'), icon: <TrendingUp size={18} /> },
    { id: 'command-center', label: t('sidebar.command_center'), icon: <Terminal size={18} /> },
    { id: 'agent', label: t('sidebar.agent_chat'), icon: <Bot size={18} /> },
    { id: 'db-agent', label: t('sidebar.db_agent'), icon: <Database size={18} /> },
    { id: 'about', label: t('sidebar.about_oaktree'), icon: <Info size={18} /> },
  ];

  return (
    <Box sx={{ width: '100%', mb: { xs: 2.5, md: 3.5 } }}>
      ...
      >
        <Stack direction="row" spacing={{ xs: 1.5, md: 2.5 }} alignItems="center">
          ...
          >
            <OaktreeIcon color="white" size={24} />
          </Box>
          <Box>
            <Typography
              level="h2"
        ...
        <Stack direction="row" spacing={{ xs: 1, md: 1.5 }} alignItems="center">
          <Box sx={{ display: 'flex', gap: { xs: 1, md: 1.25 }, alignItems: 'center' }}>
            ...
            <Tooltip title={mounted && mode === 'dark' ? t('header.light_mode') : t('header.dark_mode')} placement="bottom">
              ...
              >
                {mounted && mode === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
              </IconButton>
            </Tooltip>
            ...
            <Box sx={{ display: { xs: 'none', md: 'block' }, width: '1px', height: '24px', bgcolor: 'divider', mx: 0.5 }} />
            ...
            <Dropdown>
              ...
              >
                ...
                <Box sx={{ display: { xs: 'none', sm: 'block' }, textAlign: 'left' }}>
                  <Typography level="body-sm" sx={{ fontWeight: 700, color: 'text.primary', fontSize: '0.825rem' }}>
                    {user?.name || t('header.operator')}
                  </Typography>
                  <Typography level="body-xs" sx={{ opacity: 0.5, fontWeight: 500, fontSize: '0.7rem' }}>
      ...
      {/* ROW 2: Sticky Desktop Navigation Tabs */}
      <Box
        sx={{
      ...
      >
        <Sheet
          sx={{
            ...
            border: mode === 'dark' ? '1px solid rgba(255, 255, 255, 0.12)' : '1px solid rgba(0, 0, 0, 0.07)',
